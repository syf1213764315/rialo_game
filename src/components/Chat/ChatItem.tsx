...
export default function ChatItem(props: {
  ...
  return (
    <div className="flex flex-col rounded-[8px] mb-7">
      <div className={`flex flex-row items-start p-4 justify-between rounded-t-lg ${router.asPath !== "/room/operators-hub" ? "bg-[#1a1918]" : "bg-[#0c5c63]"}`}>
        <div className="flex items-center">
          ...
          <div className="flex flex-col ml-2">
            <p
              ...
              style={{ color: base58ToGradient(props.name).color }}
            >
              {props?.name?.length > 10
                ? props.name.slice(0, 3) + "..." + props.name.slice(-3)
                : props.name}
            </p>
